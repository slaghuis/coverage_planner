 ...
 * area.  Applicable in photogrammetry, crop spraying and serch use cases.
 ***************************************************************************/
...
class CoveragePathPlannerServer : public rclcpp::Node
{
public:
  using CoveragePath = coverage_planner_interfaces::action::ComputeCoveragePath;
  using GoalHandleCoveragePath = rclcpp_action::ServerGoalHandle<CoveragePath>;

  explicit CoveragePathPlannerServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : Node("coverage_path_planning_server", options)
  {
    ...
    this->declare_parameter<float>("minimum_height", 5.0);  // Minimum Fleight Height
    this->declare_parameter<float>("maximum_height", 30.0);  // Maximum Fleight Height
            
    this->action_server_ = rclcpp_action::create_server<CoveragePath>(
      this->get_node_base_interface(),
      this->get_node_clock_interface(),
      this->get_node_logging_interface(),
      this->get_node_waitables_interface(),
      "coverage_server/compute_coverage_path",
      std::bind(&CoveragePathPlannerServer::handle_goal, this, _1, _2),
      std::bind(&CoveragePathPlannerServer::handle_cancel, this, _1),
      std::bind(&CoveragePathPlannerServer::handle_accepted, this, _1));
  }

private:
  rclcpp_action::Server<CoveragePath>::SharedPtr action_server_;
  ...
  rclcpp_action::GoalResponse handle_goal(
    ...
    std::shared_ptr<const CoveragePath::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse handle_cancel(
    const std::shared_ptr<GoalHandleCoveragePath> goal_handle)
  {
  ...
  void execute(const std::shared_ptr<GoalHandleCoveragePath> goal_handle)
  {
    RCLCPP_DEBUG(this->get_logger(), "Planning a path over the provided polygon.");
    rclcpp::Rate loop_rate(1);
    const auto goal = goal_handle->get_goal();
    auto result = std::make_shared<CoveragePath::Result>();
    ...
    do {
      ...
      do {
        ...
        waypoint_pose.position.x = waypoint.first;
        waypoint_pose.position.y = waypoint.second;
        waypoint_pose.position.z = h;
                
        waypoint_pose.orientation.x = (double) q.x();
        waypoint_pose.orientation.y = (double) q.y();
        waypoint_pose.orientation.z = (double) q.z();
        waypoint_pose.orientation.w = (double) q.w();
        
        result->path.poses.push_back(waypoint_pose);
  ...
  void handle_accepted(const std::shared_ptr<GoalHandleCoveragePath> goal_handle)
  {
    using namespace std::placeholders;
    // this needs to return quickly to avoid blocking the executor, so spin up a new thread
    std::thread{std::bind(&CoveragePathPlannerServer::execute, this, _1), goal_handle}.detach();
  }
};  // class CoveragePathPlannerServer

int main(int argc, char ** argv)
{
  rclcpp::init(argc, argv);

  auto action_server = std::make_shared<CoveragePathPlannerServer>();
